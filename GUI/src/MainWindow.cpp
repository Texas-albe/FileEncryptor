...
void MainWindow::applyPanelTransparency() {
    ...
    for(QWidget* c:{static_cast<QWidget*>(m_chkForce), static_cast<QWidget*>(m_chkVerbose),
...
QWidget* MainWindow::buildCenterPanel() {
    ...
    m_sourceCombo->addItem(tr("完成后删除源文件 (-de)"),1);
...
void MainWindow::updateAsymVisibility() {
    ...
    // 本函数会改变压缩行显隐 / 模式 / 动作等影响 argv 的状态，必须同步刷新预览；
    // 放在此处统一裁决，避免每个调用点各自补一次（onRetryCliDetection 曾漏掉，导致
    // 重定位 CLI 后预览仍显示旧路径）。
    refreshCommandPreview();
}
...
// ---------- 密钥轮换（v6 容器 rewrap） ----------
// 旧口令走 --key-stdin（管道，与 CLI 非交互通道一致），新口令写入临时密钥文件走
// --new-key-file；载荷密文不动，仅重裹 DEK。
...
void MainWindow::finishTaskRecord(const CommandResult& r) {
    ...
    // 优先用帧/逐文件精确计数；无帧场景（单文件/串行）回退到「文件开始标记数」
...
void MainWindow::refreshCommandPreview() {
    // 只在不运行时刷新预览（运行中不打断输出回显）。
    // 判据不能用「输出区文本是否以 >>> 开头」这类文本嗅探：一旦输出区出现不以 >>>
    // 开头的内容（如历史写入失败警告先落盘），预览就会永久停止刷新。
    // 预览区是"下一次将要执行的命令"的展示位，故一律整块重绘。
