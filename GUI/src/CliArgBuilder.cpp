...
QStringList CliArgBuilder::buildArguments(const ShellOptions& o) {
    ...
    // 压缩：开关与级别分离。开关 -zstd 是布尔型、不带任何参数值（CLI 侧与 -z/--compress 等价）；
    // 只有需要非默认级别时才追加取值型参数 --compression-level <N>。
    // 顺序上先布尔开关后级别，CLI 两次赋值后者生效，语义稳定。
...
QString CliArgBuilder::buildPreview(const QString& programPath, const ShellOptions& o) {
    ...
    // 也无法覆盖未来新增的取值 flag。
    // 注意：预览仅供展示；实际执行走 buildArguments() 交 QProcess 逐参数传递，不经过本引号层。
    QString cmd = QStringLiteral("\"%1\"").arg(QDir::toNativeSeparators(programPath));

    // 取值型 flag 清单：其后紧跟的那一项是参数值（需要加引号）。
    // 布尔开关（如 -zstd / -de / -y / -v / --sha256 / --restore-name）**不得**列入，
    // 否则其后的独立参数会被误判成开关的取值而被错误加引号。
