// FontBootstrap 实现（详见 FontBootstrap.h 顶部说明）
...
bool hasCjkGlyph(const QFont& f) {
    QFontMetrics fm(f);
    // inFont 不做字体回退，只问这是一款字体自身是否有该字形
    return fm.inFont(QChar(kProbeChar));
}

// 候选字体族：按“简中桌面常见度”排序，跨平台都列全（Windows/macOS 一般走不到这里，
// 因为它们的默认字体本身就有中文）。
...
// 从构建期嵌入的资源字体里挑一款含中文的（Qt 资源 :/fonts/，由 CMake fonts.qrc 打进 exe）
...
void FontBootstrap::initialize() {
    ...
    if (chosen.isEmpty()) {
        // 5) 都没有：不阻塞启动，仅告警（界面中文会显示为方块，需装字体）
