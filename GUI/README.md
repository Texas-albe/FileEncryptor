...
- **CMake ≥ 3.16**。
- **Qt 6.2+ Widgets**（**静态构建版本**，如 `C:\Program Files\smelibs` 这种已编译为 `.lib` 的 Qt 静态分发）。
  - 官方 Qt 在线安装器可选「Static Qt Builds」组件。
  - 自带的静态 Qt 分发（路径如 `C:\Program Files\smelibs`）开箱即用。
...
> **关键前提**：`Qt6::Core` / `Qt6::Gui` / `Qt6::Widgets` 必须是 **STATIC IMPORTED** 目标（`add_library(Qt6::Core STATIC IMPORTED)`，见 `lib/cmake/Qt6Core/Qt6CoreTargets.cmake`）。动态构建的 Qt（官方 msvc2022_64 默认）会链接到 `Qt6Core.dll`，不是本项目目标。本项目默认只支持**静态 Qt 构建**；如使用动态 Qt，构建可成功但运行时需把 Qt DLL 放在 exe 同目录（与本项目"不依赖 Qt DLL"目标不符）。

> **平台插件 / 图像格式插件**：通过 `src/main.cpp` 中的 `Q_IMPORT_PLUGIN(QWindowsIntegrationPlugin)` / `QJpegPlugin` / `QGifPlugin` / `QICOPlugin` 等声明静态导入，链接器把插件代码静态打入 exe，运行时无需 `platforms/qwindows.dll` 或 `imageformats/qjpeg.dll` 等。
