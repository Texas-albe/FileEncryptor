...
- **CMake ≥ 3.15**。
- **libsodium ≥ 1.0.19**（AEGIS-256 需要；1.0.22 及以上推荐）。预编译库已随仓库放于 `../third_party/libsodium/`，CMake 默认优先使用；系统安装（Linux 发行版包 / vcpkg）作为回退。CMake 会在配置阶段校验版本，过低会给出明确报错。
- **yaml-cpp**（YAML 配置解析，v1.7.2 起依赖）。源码与预编译静态库（/MT）已随仓库放于 `../third_party/yaml-cpp/`（`include/` + `src/` + `lib/yaml-cpp.lib`），CMake 默认优先使用；也可用系统安装（Linux `libyaml-cpp-dev`，Windows 标准布局或 `YAMLCPP_ROOT` 指定）。
...
选项：
  ...
  --rename           -R 配合：把 .ptd 自身原地重命名为 <原始名>.ptd（内容不变，目标已存在则拒绝以防覆盖）。
...
本机 age 身份与收件人公钥的统一管理层，存放于**用户配置目录**下的 `keys/` 子目录（Windows 为 `%APPDATA%\FileEncryptor\keys`，Linux 为 `~/.config/fileencryptor/keys`）。目录内含索引 `library.yaml`（仅元数据：名称 / 类型 / 别名 / 备注 / 创建时间 / 缓存公钥）与每把密钥一个 `<名称>.key` 材料文件。GUI 与 CLI 共用同一布局，任一端导入的密钥对另一端立即可见。

- **`-L add <密钥文件> --as <名称> [--alias ...] [--notes ...]`**：复制材料入库并按内容自动识别类型（`AGE-SECRET-KEY-...` 前缀为身份，`age1...` 为收件人）。导入身份时会立即派生对应公钥缓存进索引，之后加密不再需要接触私钥。重名拒绝导入；材料文件写入后权限收紧为仅拥有者可读。
- **`-L list` / `-L show <名称>`**：查看库内容与条目详情；密钥材料永不打印，需用 `-L export` 导出。
- **`-L pub <名称|文件>`**：从身份私钥派生公钥并打印；参数为库名称时会同时把结果缓存进索引（`-K` 加密即依赖此缓存）。
- **`-L export <名称> [目录]`**：把材料文件复制出库；目标同名文件已存在时拒绝覆盖。
- **`-L remove <名称>`**：删除索引条目与材料文件。
