...
// 功能14：加密强度预设（快速 / 标准 / 加固）。
//   0=fast    : ops=3, mem=64MB  （兼容旧格式文件）
//   1=standard: ops=4, mem=128MB （默认，当前写出值）
//   2=strong  : ops=6, mem=512MB （抗暴力破解更强，速度更慢）
// 参数写入文件头，解密端自适应（v2+ 已支持按文件头参数化派生）。
static void kdf_preset_params(int preset, unsigned int& ops, unsigned int& mem_kb) {
    switch(preset) {
        case 0:  ops=3; mem_kb=64*1024;   break;   // fast
        case 2:  ops=6; mem_kb=512*1024;  break;   // strong
        case 1:
        default: ops=ARGON2_OPS_DEFAULT; mem_kb=ARGON2_MEM_DEFAULT_KB; break; // standard
    }
}

// ---------- KDF 参数安全上界（v2.1.2，防恶意密文头触发资源耗尽 DoS） ----------
// 文件头里的 opslimit / memlimit_kb 是**攻击者可控**的：v4 的 header_hmac 在 KDF 之后
// 才校验，所以在拿到密钥之前无法区分"正常文件"与"伪造头"。若不加上界，一个 125 字节的
// 恶意 .ptd 就能让解密方跑上百秒并吃掉数 GB 内存（批量 -bd 还会按文件数线性放大）。
// 本程序自己写出的头恒为 ops=4 / mem=128MB（旧格式 ops=3 / mem=64MB），上界留有余量。
static constexpr unsigned int KDF_OPS_LIMIT_MAX=10;
static constexpr size_t       KDF_MEM_LIMIT_MAX_BYTES=size_t(1)<<30;  // 1 GiB

// 缺陷7：批量解密时单文件 KDF 上界 1 GiB，但并发线程数可达 CPU 核数，
// N 线程同时跑大内存 Argon2 可能 OOM。用进程级信号量限制"同时进行 KDF"的线程数，
// 把总 KDF 内存约束在 YAML 的 max_memory_bytes 内。预算 = max(1, max_memory_bytes / 128MB)；
// max_memory_bytes==0 表示不限制（由线程数本身约束）。惰性初始化，首次 KDF 时按配置定容。
namespace {
struct KdfSemaphore {
    std::mutex mtx;
...
// v5 头部：在 v4 基础上于 iv 之后追加 2 字节压缩描述（compression / comp_level），
// 磁盘格式升级为 v5（仅当启用压缩时写出；不压缩仍写 v4，保持与旧版完全一致的字节布局）。
//   compression : 0 = 无压缩；1 = zstd
//   comp_level  : zstd 压缩级别（1..22 常规；负数 -1..-5 为快速档），仅 compression==1 时有效
// 每数据块在 v5+压缩 下额外带 4 字节小端“压缩帧长度”前缀，故磁盘块为变长；
// 解密端按头部 compression 标记决定读取方式，与加密端逐块一一对应（续传安全）。
#pragma pack(push, 1)
struct FileHeaderV5 {
    unsigned char magic[4];
    ...
    unsigned char comp_level;      // zstd 级别（仅 compression==1）
    unsigned char plaintext_hash[HASH_SIZE];
    unsigned char header_hmac[HEADER_HMAC_SIZE];
};
#pragma pack(pop)
static constexpr size_t HEADER_SIZE_V5=sizeof(FileHeaderV5); // 143
// v5 头部 HMAC 覆盖区域：前 79 字节（magic..comp_level，含压缩标记）
static constexpr size_t HEADER_HMAC_COVER_V5=HEADER_SIZE_V5-HASH_SIZE-HEADER_HMAC_SIZE; // 79
...
// 小端 32 位读写（v5+压缩块前缀：4 字节压缩帧/密文长度）
...
// 文件头 HMAC 覆盖字节数（按版本：v5 覆盖到压缩标记，其余覆盖到 iv）
static inline size_t header_hmac_cover(unsigned char ver) {
...
// ---------- 续传进度文件绑定标识（防重放） ----------
// 将"正在处理的源文件"的规范化路径 + 大小 + mtime 纳入 .prs 的 HMAC 输入，
// 使旧的有效 .prs 无法被重放到不同的文件（路径/mtime/size 任一变化即 HMAC 失配）。
...
// 内容令牌（防重放增强）：对源文件"头部 64KB + 尾部 64KB + 总大小"求 Blake2b，
// 折叠为一段稳定指纹。攻击者若仅用 `touch -r` 复刻 mtime 并保持大小（不改动内容），
// 旧 .prs 仍会因内容令牌失配而 HMAC 失败，无法重放到内容不同的文件。
// 整文件小于 128KB 时直接对全文求哈希；不读取整文件，故对大文件开销恒定。
...
static std::string compute_progress_binding(const std::string& in_path) {
    ...
    char buf[256];
    int n = snprintf(buf, sizeof(buf), "|%s|%lld|%lld|%s",
        norm.c_str(), (long long)sz, (long long)mt, token.c_str());
...
// 便于与外部备份 / 传输工具链配合校验传输完整性。由 --sha256 或 YAML write_sha256 启用。
...
// 用 C++11 局部 static 初始化（magic static）保证线程安全：并发批量任务首次调用时
// 不会因"先读后写 cached"产生数据竞争（历史实现 static int cached 非线程安全）。
...
// v2.1.2：去掉 static —— 非对称（rage）分支此前完全绕过路径策略，现由 run_asym 调用。
...
// 收紧密钥文件权限（仅拥有者可读）。Windows 写入真正的 DACL：只保留当前用户
// + SYSTEM，且 PROTECTED_DACL 阻断继承，达到 0600 语义（v2.1.2 前的
// _chmod(_S_IREAD) 只是"只读"属性位，并非访问控制）。ACL 失败时静默回退。
...
static bool derive_key(const unsigned char* password, size_t pwd_len,
    ...
    // v2.1.2：统一在此处校验攻击者可控的 KDF 参数（唯一派生入口，覆盖解密 v2/v3/v4、
    // read_original_name 与加密续传分支）。超出上界说明文件头被篡改或伪造，
    // **直接拒绝**而不是降级——降级仍会白白跑一次 1 GiB 的 Argon2（实测约 13 s），
    // 批量解密时照样能被放大成 DoS；拒绝则零资源消耗。
    ...
    // 进程级 KDF 内存预算：并发进入 Argon2 前先取信号量许可，避免批量解密 OOM。
    g_kdf_sem.ensure();
    g_kdf_sem.acquire();
...
// 因此中断后续传仍能命中同一输出文件与 .prs，同时不泄露原始文件名。
...
// Windows 保留设备名（忽略扩展名比较基名）：CON PRN AUX NUL COM1..COM9 LPT1..LPT9。
// 还原出的文件名若命中这些，会在打开/创建时指向设备而非文件，造成越权或异常。
...
bool read_original_name(const std::string& ptd_path, std::string& out_name,
    ...
    if(ver!=1&&ver!=2&&ver!=3&&ver!=VERSION&&ver!=5) return false;
...
bool read_ptd_metadata(const std::string& ptd_path, PtdMeta& meta) {
    ...
    std::ifstream f;
    if(!open_stream(f,ptd_path,std::ios::binary)) return false;
    unsigned char hdr[HEADER_SIZE_V5];
    ...
    else if(ver==2) { FileHeader*  h=reinterpret_cast<FileHeader*>(hdr);  salt_ptr=h->salt; meta.mode=static_cast<CryptoMode>(h->mode); meta.opslimit=h->opslimit; meta.memlimit_kb=h->memlimit_kb; iv_len=h->iv_len;
                     char hex[65]; sodium_bin2hex(hex,sizeof(hex),h->iv,std::min<size_t>(h->iv_len,24)); meta.iv_hex=hex; }
...
bool encrypt_file(const std::string& in_path,
    ...
    if(out_exists) {
        ...
        if(open_stream(fhex,out_path,std::ios::binary)) {
            unsigned char ehbuf[HEADER_SIZE_V5];
            if(fhex.read(reinterpret_cast<char*>(ehbuf),HEADER_SIZE_V5)
                ...
                if(ver==3||ver==4||ver==5) {
                    ...
                    existing_is_v3=true;
                    if(ver==5) {
                        if(fhex.gcount()==HEADER_SIZE_V5) {
                            existing_is_v5=true;
                            memcpy(existing_hdr_hmac,ehbuf+HEADER_SIZE_V5-HEADER_HMAC_SIZE,HEADER_HMAC_SIZE);
                            existing_compression=ehbuf[77];
                            existing_comp_level=(int)(signed char)ehbuf[78];
                        }
                    } else if(ver==4) {
                        existing_is_v4=true;
    ...
    // 静默抹掉，且 .prs 随后被删，造成数据永久丢失（批量 -be 无交互确认即触发）。
    // 仅当原输出不存在（孤立 .prs）时才安全从头重做。
    ...
    (void)compress_level;
#endif
    const size_t hdr_cover = do_compress ? HEADER_HMAC_COVER_V5 : HEADER_HMAC_COVER;
    const bool resume_compressed = (has_progress && existing_is_v5 && existing_compression==1);

    size_t existing_hdr_size = existing_is_v5 ? HEADER_SIZE_V5
    ...
    // 否则校验失败时输出已被截断，随后的 cleanup 会删除 .ptd 与 .prs，
    ...
    if(!header_written) {
        ...
        header.version = do_compress ? 5 : VERSION;   // 启用压缩时升级为 v5（额外 2 字节压缩描述）
        ...
        // AAD 覆盖文件头前 77 / 79 字节（v5 含压缩标记）；不含 plaintext_hash 与 header_hmac
        ...
        // 文件头 HMAC 在写头瞬间即计算并随头落盘：覆盖前 77 / 79 字节（v5 含压缩标记，
        // 含 salt/mode/Argon2 参数/iv），不含 plaintext_hash/header_hmac 自身。
        // 这样被中断的半成品 .ptd 也已携带合法 header_hmac，续传时“文件头篡改校验”可正常通过；
        // 同时防攻击者替换 salt/iv/mode 后重放旧 .prs。
        {
            unsigned char hdr_auth[HEADER_HMAC_SIZE];
            derive_header_auth_key(key.data(),hdr_auth);
    ...
    if(ok) {
        // 结束哈希并回填 plaintext_hash；文件头 HMAC 已在写头瞬间算好（覆盖前 77/79 字节），
        // 此处仅重算一遍（前缀未变，结果一致）后随 plaintext_hash 一并写回。
        ...
        crypto_auth(header.header_hmac,
            reinterpret_cast<const unsigned char*>(&header),hdr_cover,hdr_auth);
        fout.flush();
        size_t hdr_size = do_compress?HEADER_SIZE_V5:HEADER_SIZE_V4;
...
bool decrypt_file(const std::string& in_path,
    ...
    // v4/v5 文件头 HMAC 校验：独立于 AEAD，防止文件头被篡改（如替换 salt/iv/mode 后重放）。
    ...
    size_t aad_hdr_len=header_hmac_cover(ver);
    ...
    bool use_increment=(ver==VERSION || ver==5);
...
bool process_files(const std::vector<std::string>& input_paths,
    ...
    // 显示线程：固定 80ms 一帧（progress_ui 内部还会再节流一层），
    // 使空闲线程行也能被稳定刷新，而不依赖 worker 的进度回调频率。
