...
### Fixed
- **Windows 密码退格逻辑回归（v1.4.3 引入）**：`get_password_win()` 的 `pop_utf8` lambda 中循环起点误用 `pwd.size()`（应为 `pwd.size()-1`），导致：① ASCII 密码退格**完全失效**（`resize(size())` 等于无操作，回归到比 v1.4.2 的 `pop_back()` 更差）；② UTF-8 多字节密码退格仍保留孤立首字节、破坏序列。已改为从末尾字节起步、检查 `pwd[i]` 并向前跨过续字节（`0x80–0xBF`），停在首字节，`resize(i)` 删除整个字符。已用 Python 复刻验证 ASCII（`Str0ng`→`Str0n`）与中文（`ab你`→`ab`）两场景均正确，并经 MSVC 编译验证零错误。

---

## [1.4.3] - 2026-08-19（密码非交互可用性 + 批量健壮性 + 性能/显示修复）

### Fixed
- **[中/高] Windows 密码输入在非交互环境完全不可用**：`get_password_win()` 原本无条件用 `_getch()` 读控制台缓冲区，管道喂密码会挂起（直到 timeout），经 winpty PTY 桥接会误报 `stdin is not a tty` 后不真正执行。现先检测 stdin 是否为控制台（`GetConsoleMode`），非控制台（管道 / 重定向 / PTY）时退化为从 `std::cin` 读取一行（UTF-8 字节），与 POSIX 路径行为一致。
- **[中/高] Windows 退格破坏 UTF-8 序列**：`_getch()` 逐字节累积（多字节中文密码本就靠多次调用累积），但退格原用 `pop_back()` 按字节删除，会破坏多字节序列。现改为按 UTF-8 码点（去掉末尾连续续字节）删除。
- **[中] 批量解密 `-bd` 缺少 `.ptd` 过滤**：解密分支原本对目录下所有文件都尝试解密，非密文文件（readme.txt 等）magic 校验失败后会全部计入 “Decryption errors” 误导用户，且可能误处理巧合以 `FENC` 开头的文件。现解密时只处理 `.ptd` 文件（与单文件 `-d` 强制 `.ptd` 一致），其余跳过并汇总计数。
- **[中] 批量解密跨多个 `-i` 根目录输出路径碰撞**：`build_batch_out_path` 解密时（`include_root_name=false`）`dirA/x` 与 `dirB/x` 会映射到同一输出路径，多线程并行下第二个线程被跨进程锁判为 “locked”。现当存在多个输入根目录（`input_paths.size()>1`）时，即使是解密也附加根目录名前缀做消歧；单根目录解密布局保持不变。
- **[低,显示] 批量进度条在失败文件上仍显示到 100%**：进度补齐逻辑原本无条件执行，失败文件也会把进度条拉满。现仅对成功处理的文件补齐进度。

### Changed
- **[低,潜伏] 删除死代码 `build_aad_with_metadata(const FileHeader&)`**：该重载用定长 `HEADER_SIZE`(69) 截取头部，对 v3(109 字节，含 32 字节 plaintext_hash) 是错误的；当前所有调用都走指针重载（`HEADER_SIZE_V3-HASH_SIZE`），此函数从未被调用。一旦将来误接会导致 AAD 少 32 字节、v3 解密静默失败，故直接删除。
- **[低,性能] 加密后自校验不再重复派生 Argon2**：`encrypt_file` 每文件加密后用 `decrypt_file` 做自解密验证，而原 `decrypt_file` 会再跑一次 128MiB 的 `crypto_pwhash`，等于每个文件 KDF 成本翻倍。现 `decrypt_file` 新增可选参数 `ext_key`（已派生密钥），自校验复用本次已派生的 `key`，密钥清零也顺延到自校验之后。大批量加密时明显提速，正确性不受影响。
