# ChangeLog - FileEncryptor GUI (Qt6 / Linux)

本文件记录 **FileEncryptor GUI (Qt6 / Linux)** 子项目的所有重要变更（图形界面 `FileEncryptorGUI(.exe)`）。
...
版本号与 Windows 侧（WinUI 3）对齐，产物命名加入框架标识（配套 CLI 期望版本 2.4.2）。

### Changed
- **版本号**：升至 2.0.0。
- **产物命名**：Linux 可执行文件改为 `FileEncryptorGUI-2.0.0-Qt-Linux`，DEB/RPM 包名改为 `file-encryptor-gui-qt`。
...
### Build
- Linux 构建现在和 CLI 一样，链接完成后自动把规范命名的二进制拷到 `out/Ubuntu-26.04/build/linux-release/bin/`，并跑 cpack 产出 DEB/RPM 到 `out/packages/`，不用再手动进构建目录敲 cpack。经 VS+WSL 构建时产物直接落到 E 盘可见目录。
- Qt6 的查找路径按平台分开了：Windows 走仓库里的 `../third_party/smelibs`，WSL/Linux 优先 `/opt/smelibs`（没有就回退系统 Qt）。之前两边都塞同一条 Windows 路径，WSL 里会误捡到 moc.exe 导致构建失败。
- CPack 配置不再包在 `if(UNIX)` 里，和 CLI 保持一致；`install()` 补了执行权限，打出来的包装上后二进制可直接运行。
- CMakePresets 新增 `windows-vs2026-release` / `windows-vs2026-debug`，VS2026 打开文件夹选这两个预设就能直接编译调试，不用再走 Ninja。
