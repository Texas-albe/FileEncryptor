...
TaskHistoryDialog::TaskHistoryDialog(QWidget* parent) : QDialog(parent) {
    ...
    m_btnDelete=new QPushButton(tr("删除选中"));
    ...
    connect(m_btnDelete,&QPushButton::clicked,this,&TaskHistoryDialog::onDeleteSelected);
    m_table->setContextMenuPolicy(Qt::CustomContextMenu);
    connect(m_table,&QTableWidget::customContextMenuRequested,this,&TaskHistoryDialog::onTableContextMenu);
...
void TaskHistoryDialog::onDeleteSelected() {
    const int row=m_table->currentRow();
    if(row<0||row>=m_records.size()) {
        MsgBox::info(this,tr("删除"),tr("请先选择要删除的记录。"));
        return;
    }
    const TaskRecord& r=m_records[row];
    if(!MsgBox::confirm(this,tr("删除记录"),
        tr("确定删除这条历史记录？\n\n%1  %2").arg(r.startedAt).arg(r.actionLabel))) return;
    m_records.removeAt(row);
    QString err;
    if(!TaskHistory::save(m_records,err)) {
        MsgBox::warn(this,tr("删除失败"),tr("无法保存历史：%1").arg(err));
        return;
    }
    reload();
}

void TaskHistoryDialog::onTableContextMenu(const QPoint& pos) {
    const int row=m_table->rowAt(pos.y());
    if(row<0) return;
    m_table->selectRow(row);
    QMenu menu(this);
    menu.addAction(tr("删除此条"),this,&TaskHistoryDialog::onDeleteSelected);
